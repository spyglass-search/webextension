...
const App = () => {
    ...
    let [bookmarks_last_synced, set_bookmarks_last_synced] = useState<Date | null>(null);
    let [history_last_synced, set_history_last_synced] = useState<Date | null>(null);
      ...
      browser.tabs
        ...
        .then((tabs: Array<BrowserTabs.Tab>) => {
          if (tabs[0]?.url) {
            ...
            return SPYGLASS_CLIENT.is_document_indexed(tabs[0].url)
              .then(result => set_is_indexed(result));
          }
        });

      getStore<Date>(StoreKeys.BookmarksSyncTime)
        .then(value => set_bookmarks_last_synced(value));

      getStore<Date>(StoreKeys.HistorySyncTime)
        .then(value => set_history_last_synced(value));

    }, []);


    let tabClass = "cursor-pointer border-b-2 border-neutral-900 px-4 pb-2 text-neutral-400 hover:border-cyan-500 hover:text-white focus:outline-none";
    return (
